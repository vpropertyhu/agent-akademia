...
export default async function(request:Request,context:Context){
 try{
  ...
  if(request.method==='GET'&&new URL(request.url).searchParams.get('capabilities')==='research')return reply({configured:!!Netlify.env.get('OPENAI_API_KEY')&&(Netlify.env.get('AGENT_API_SECRET')||'').length>=32});
  ...
  if(request.method==='GET'){const id=new URL(request.url).searchParams.get('id');if(id){if(!uuid(id))return reply({error:'Érvénytelen azonosító.'},400);const w=await store.get(key(owner,id),{type:'json'}) as Work|null;return w&&w.owner===owner?reply({work:visible(w)}):reply({error:'A munka nem található.'},404);}const wantsResearch=new URL(request.url).searchParams.get('assistant')==='research';const [workspace,configured]=await Promise.all([readWorkspace(store,owner),wantsResearch?Promise.resolve(!!Netlify.env.get('OPENAI_API_KEY')&&!!secret&&secret.length>=32):connected(api,secret)]);return reply({...workspace,configured});}
  ...
  if(!secret||secret.length<32||(raw.job==='kutato'?!Netlify.env.get('OPENAI_API_KEY'):!await connected(api,secret)))throw new AIError('NOT_CONFIGURED','Az AI-szolgáltatás még nincs csatlakoztatva, vagy éppen indul. A mentett munkák elérhetők; próbáld újra később.',503);
