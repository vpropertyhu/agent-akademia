const f = (key, label, type = 'text', hint = '', required = true) => ({ key, label, type, hint, required });
const choice = (key, label, options) => ({ ...f(key, label, 'select'), options, default: options[0] });
const dateField = f('today', 'Viszonyítási nap', 'date');
const list = (label, hint) => f('items', label, 'textarea', hint);
...
export function day(value) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(value) || value < '1900-01-01' || value > '2100-12-31')
        throw new Error('A dátum formátuma ÉÉÉÉ-HH-NN legyen (1900–2100).');
    const n = Date.parse(value + 'T12:00:00Z');
    if (!Number.isFinite(n) || new Date(n).toISOString().slice(0, 10) !== value)
        throw new Error('Nem létező dátum: ' + value);
    return n;
}
const shift = (value, n) => new Date(day(value) + n * 86400000).toISOString().slice(0, 10);
export function initialInput(tool) { return Object.fromEntries(tool.fields.map(x => [x.key, x.default ?? (x.type === 'date' ? localDate() : '')])); }
export function sampleInput(tool, today = localDate()) { return Object.fromEntries(Object.entries(tool.sample).map(([k, v]) => [k, v.replaceAll('@today', today).replaceAll('@next', shift(today, 1)).replaceAll('@yesterday', shift(today, -1))])); }
export function getTool(id) { const t = dailyTools.find(t => t.id === id); if (!t)
    throw new Error('Ez a segéd nem található.'); return t; }
export function number(value, label = 'Szám', min = 0, max = 1e12) {
    const clean = value.replace(/[ \u00a0\u202f]/g, '').replace(',', '.');
    if (!/^-?\d+(\.\d+)?$/.test(clean))
        throw new Error(`${label}: érvényes számot adj meg, ezreselválasztó pont nélkül.`);
    const n = Number(clean);
    if (!Number.isFinite(n) || n < min || n > max)
        throw new Error(`${label}: ${min} és ${max} közötti szám szükséges.`);
    return n;
}
const fmt = (n) => new Intl.NumberFormat('hu-HU', { maximumFractionDigits: 2 }).format(n);
const round = (n) => Math.round((n + Number.EPSILON) * 100) / 100;
const lines = (value) => value.split(/\r?\n/).map(x => x.trim()).filter(Boolean);
const bullets = (value) => lines(value).map(x => '• ' + x).join('\n');
const normalize = (value) => value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().toLowerCase();
const url = (value) => { let u; try {
    u = new URL(value);
}
catch {
    throw new Error('Teljes http:// vagy https:// URL-t adj meg.');
} if (!['http:', 'https:'].includes(u.protocol) || u.username || u.password)
    throw new Error('A link csak http/https lehet, beágyazott belépési adat nélkül.'); return u; };
const curr = (s) => { const c = s.toUpperCase(); if (!['HUF', 'EUR', 'USD'].includes(c))
    throw new Error('A pénznem HUF, EUR vagy USD lehet.'); return c; };
const member = (value, options, label) => { const index = options.map(normalize).indexOf(normalize(value)); if (index < 0)
    throw new Error(`${label}: ${options.join(', ')} valamelyike szükséges.`); return options[index]; };
/** RFC-style quoted fields; delimiter is selected outside quotes. Never silently drop malformed rows. */
export function parseTable(raw, columns) {
    ...
    const addRow = () => { addCell(); if (row.some(Boolean))
        result.push(row); row = []; };
    for (let i = 0; i < text.length; i++) {
        const ch = text[i];
        if (quoted) {
        ...
        if (ch === '"') {
            if (cell.trim() || closed)
                throw new Error('Hibás idézőjel a táblázatban.');
            cell = '';
            quoted = true;
            continue;
        }
        if (closed && ch.trim())
            throw new Error('Az idézőjel után csak oszlopelválasztó következhet.');
        cell += ch;
    }
    if (quoted)
        throw new Error('Lezáratlan idézőjel a táblázatban.');
    addRow();
    if (result[0] && normalize(result[0][0]) === normalize(columns[0]))
        result.shift();
    if (!result.length)
        throw new Error('Legalább egy adatsort adj meg a fejléc után.');
    if (result.length > 500)
        throw new Error('Egyszerre legfeljebb 500 sor dolgozható fel.');
    result.forEach((r, i) => { if (r.length !== columns.length)
        throw new Error(`${i + 1}. adatsor: ${columns.length} oszlop szükséges, ${r.length} érkezett. Üres mezőnél is tartsd meg az elválasztót.`); if (!r[0])
        throw new Error(`${i + 1}. adatsor: az első oszlop nem lehet üres.`); });
    return result;
}
export function validateInput(tool, input) {
    ...
    for (const field of tool.fields) {
        ...
        if (typeof rawValue !== 'string')
            throw new Error('Szöveges mezők szükségesek.');
        const value = rawValue.trim();
        if (field.required !== false && !value)
            throw new Error(`Töltsd ki ezt a mezőt: ${field.label}.`);
        if (value.length > 20000)
            throw new Error(`${field.label}: legfeljebb 20 000 karakter.`);
        if (value) {
            if (field.type === 'number')
                number(value, field.label, field.min ?? 0, field.max ?? 1e12);
            if (field.type === 'date')
                day(value);
            if (field.type === 'url')
                url(value);
            if (field.options && !field.options.includes(value))
                throw new Error(`Válassz a listából: ${field.label}.`);
        }
        out[field.key] = value;
    }
    return out;
}
const result = (title, text, notes = []) => ({ title, text, notes });
const table = (title, columns, rows, notes = []) => ({ title, columns, rows, notes, text: [title, '', columns.join('\t'), ...rows.map(r => r.join('\t')), ...(notes.length ? ['', ...notes] : [])].join('\n') });
export function runTool(id, raw) {
    const tool = getTool(id), v = validateInput(tool, raw), read = (cols) => parseTable(v.items, cols);
    switch (id) {
        case 'feladatrendezo': {
            const rows = read(['Feladat', 'Felelős', 'Határidő', 'Prioritás']).map(r => { if (r[2])
                day(r[2]); const p = number(r[3], 'Prioritás', 1, 3); if (!Number.isInteger(p))
                throw new Error('A prioritás 1, 2 vagy 3 lehet.'); return r; });
        ...
        case 'hetitervezo': {
            ...
            let cursor = v.start;
            while (days.length < 5) {
                const week = new Date(day(cursor)).getUTCDay();
                if (week !== 0 && week !== 6)
                    days.push(cursor);
                cursor = shift(cursor, 1);
            }
            const used = days.map(() => 0);
            const rows = tasks.map(t => { const i = used.findIndex(n => n + t.minutes <= capacity); if (i < 0)
                return ['Nem fér bele', t.name, fmt(t.minutes), 'Külön időpont vagy bontás szükséges']; used[i] += t.minutes; return [days[i], t.name, fmt(t.minutes), 'Beosztva']; });
        ...
        case 'megbeszeles': {
            const tasks = read(['Feladat', 'Felelős', 'Határidő']);
            tasks.forEach(r => { if (r[2])
                day(r[2]); });
        ...
        case 'erdeklodo-rendezo': {
            const rows = read(['Név', 'Elérhetőség', 'Státusz', 'Következő kapcsolat', 'Teendő']).map(r => { r[2] = member(r[2], ['új', 'folyamatban', 'lezárt'], 'Státusz'); if (r[3])
                day(r[3]); return [...r, r[2] === 'lezárt' ? 'Lezárt' : !r[3] ? 'Időpontot kell megadni' : r[3] < v.today ? 'Elmaradt kapcsolat' : r[3] === v.today ? 'Ma keresd' : 'Később']; });
            const rank = ['Elmaradt kapcsolat', 'Ma keresd', 'Időpontot kell megadni', 'Később', 'Lezárt'];
            rows.sort((a, b) => rank.indexOf(a[5]) - rank.indexOf(b[5]) || a[3].localeCompare(b[3]));
            return table(tool.name, ['Név', 'Elérhetőség', 'Státusz', 'Következő kapcsolat', 'Teendő', 'Jelzés'], rows, ['A megadott dátum és státusz alapján rendez. Nem küld üzenetet.']);
        }
        case 'kontakt-tisztito': {
            const rows = read(['Név', 'Email', 'Telefon']), map = new Map();
            let duplicates = 0, conflicts = 0;
            const out = [];
            for (const r of rows) {
                r[1] = r[1].trim().toLowerCase();
                r[2] = r[2].replace(/[\s()-]/g, '');
                const key = r[1] || 'row-' + out.length;
                const prior = map.get(key);
                if (prior) {
                    duplicates++;
                    if (!prior[2])
                        prior[2] = r[2];
                    else if (r[2] && prior[2] !== r[2]) {
                        prior[3] = 'Eltérő telefonszám; első megtartva';
                        conflicts++;
                    }
                    if (normalize(prior[0]) !== normalize(r[0])) {
                        prior[3] = 'Eltérő név; első megtartva';
                        conflicts++;
                    }
                    continue;
                }
                const row = [...r, !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r[1]) ? 'E-mail ellenőrzendő' : 'Formátum rendben'];
                map.set(key, row);
                out.push(row);
            }
        ...
        case 'ajanlatkeszito': {
            let sum = 0;
            const rows = read(['Tétel', 'Mennyiség', 'Egységár']).map(r => { const quantity = number(r[1], 'Mennyiség', 0.001, 1e6), unit = number(r[2], 'Egységár', 0, 1e9), total = round(quantity * unit); if (total > 1e12)
                throw new Error('A tétel értéke túl nagy.'); sum += total; return [r[0], fmt(quantity), fmt(unit), fmt(total)]; });
            sum = round(sum);
            const discount = round(sum * number(v.discount) / 100), net = round(sum - discount), tax = round(net * number(v.tax) / 100);
        ...
        case 'hatarido-figyelo': {
            ...
            rows.sort((a, b) => Number(a[5] === 'fizetve') - Number(b[5] === 'fizetve') || a[4].localeCompare(b[4]));
            return table(tool.name, ['Partner', 'Azonosító', 'Összeg', 'Pénznem', 'Határidő', 'Állapot', 'Jelzés'], rows, ['A fizetési állapotot te adod meg; nincs banki kapcsolat.']);
        }
        case 'koltsegosszesito': {
            const groups = new Map(), totals = new Map();
            for (const r of read(['Megnevezés', 'Kategória', 'Összeg', 'Pénznem'])) {
                if (!r[1])
                    throw new Error('Minden kiadáshoz adj meg kategóriát.');
                const c = curr(r[3]), n = number(r[2], 'Kiadás', 0.01), key = JSON.stringify([r[1], c]);
                groups.set(key, (groups.get(key) || 0) + n);
                totals.set(c, (totals.get(c) || 0) + n);
            }
            return table(tool.name, ['Kategória', 'Pénznem', 'Összeg'], [...groups].map(([key, n]) => [...JSON.parse(key), fmt(round(n))]), [...totals].map(([c, n]) => `Összesen: ${fmt(round(n))} ${c}`).concat('Különböző pénznemeket nem vált át és nem ad össze.'));
        }
        case 'elofizetesek': {
            const totals = new Map();
            const rows = read(['Név', 'Díj', 'Pénznem', 'Gyakoriság']).map(r => { const n = number(r[1], 'Díj', 0), c = curr(r[2]), frequency = member(r[3], ['havi', 'éves'], 'Gyakoriság'), annual = frequency === 'havi' ? n * 12 : n; totals.set(c, (totals.get(c) || 0) + annual); return [r[0], c, frequency, fmt(n), fmt(annual / 12), fmt(annual)]; });
            return table(tool.name, ['Név', 'Pénznem', 'Gyakoriság', 'Eredeti díj', 'Havi egyenérték', 'Éves egyenérték'], rows, [...[...totals].map(([c, n]) => `${c}: ${fmt(n / 12)} / hó · ${fmt(n)} / év`), 'Az egyenérték nem számlázási dátum vagy tényleges havi pénzmozgás.']);
        }
        ...
        case 'hirlevel': {
            const news = read(['Cím', 'Rövid leírás', 'Link']);
            news.forEach(r => { if (r[2])
                url(r[2]); });
            return result(tool.name, `Tárgy: ${v.subject}\n\n${v.intro}\n\n${news.map(r => `${r[0]}\n${r[1]}${r[2] ? '\n' + r[2] : ''}`).join('\n\n')}\n\n${v.cta}\n\n${v.sender}`, ['A levél szövegét készíti elő. A címzetteket és a kiküldést a saját levelezőrendszered kezeli.']);
        }
        case 'kampanylink': {
            const slug = (s) => normalize(s).replace(/\s+/g, '-');
            const rows = read(['Forrás', 'Médium', 'Tartalom']).map(r => { if (!r[1])
                throw new Error('Minden csatornához adj meg médiumot.'); const u = url(v.url); u.searchParams.set('utm_source', slug(r[0])); u.searchParams.set('utm_medium', slug(r[1])); u.searchParams.set('utm_campaign', slug(v.campaign)); if (r[2])
                u.searchParams.set('utm_content', slug(r[2]));
            else
                u.searchParams.delete('utm_content'); return [r[0], r[1], r[2], u.toString()]; });
            return table(tool.name, ['Forrás', 'Médium', 'Tartalom', 'Kampánylink'], rows, ['A linkeket hozza létre; a forgalom méréséhez a céloldalon működő analitika szükséges.']);
        }
        case 'ellenorzolista': {
            const steps = read(['Lépés', 'Felelős', 'Akkor kész, ha']);
            return result(v.title, `${v.title}\n\nINDÍTÁS\n${v.trigger}\n\nELLENŐRZŐLISTA\n${steps.map((r, i) => `☐ ${i + 1}. ${r[0]}\n   Felelős: ${r[1] || 'Kijelölendő'}\n   Akkor kész, ha: ${r[2] || 'Pontosítandó'}`).join('\n\n')}`);
        }
        case 'keszletfigyelo': {
            const rows = read(['Termék', 'Jelenlegi', 'Minimum', 'Célkészlet', 'Csomagméret']).map(r => { const current = number(r[1], 'Készlet', 0, 1e9), min = number(r[2], 'Minimum', 0, 1e9), target = number(r[3], 'Célkészlet', 0, 1e9), pack = number(r[4], 'Csomagméret', 1, 1e9); if (![current, min, target, pack].every(Number.isInteger))
                throw new Error('Egész darabszámok szükségesek.'); if (target < min)
                throw new Error(`${r[0]}: a célkészlet nem lehet kisebb a minimumnál.`); const packs = current < min ? Math.ceil((target - current) / pack) : 0; return [r[0], fmt(current), fmt(min), fmt(target), fmt(pack), fmt(packs), fmt(packs * pack), fmt(current + packs * pack)]; });
            return table(tool.name, ['Termék', 'Jelenlegi', 'Minimum', 'Cél', 'Csomagméret', 'Rendelendő csomag', 'Rendelendő darab', 'Várható készlet'], rows, ['Csak a minimum alá csökkent készlethez számol rendelést. Nem rendel meg terméket.']);
        }
        case 'naptar-export': {
            const events = read(['Cím', 'Dátum', 'Kezdés', 'Perc', 'Helyszín']);
            const escape = (s) => s.replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/;/g, '\\;').replace(/,/g, '\\,');
            const compact = (d) => d.toISOString().replace(/[-:]/g, '').slice(0, 15);
            const hash = (s) => { let h = 2166136261; for (const ch of s)
                h = Math.imul(h ^ ch.charCodeAt(0), 16777619); return (h >>> 0).toString(16); };
            const body = events.map((r, i) => { day(r[1]); if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(r[2]))
                throw new Error(`${i + 1}. sor: a kezdés ÓÓ:PP formátumú legyen.`); const duration = number(r[3], 'Időtartam', 1, 10080); if (!Number.isInteger(duration))
                throw new Error('Egész percet adj meg.'); const start = new Date(`${r[1]}T${r[2]}:00Z`), end = new Date(start.getTime() + duration * 60000); return ['BEGIN:VEVENT', `UID:${hash(r.join('|'))}-${i}@agentakademia.local`, 'DTSTAMP:20260101T000000Z', `DTSTART:${compact(start)}`, `DTEND:${compact(end)}`, `SUMMARY:${escape(r[0])}`, `LOCATION:${escape(r[4])}`, 'END:VEVENT']; }).flat();
            // Fold by UTF-8 byte length, preserving code points and the RFC 5545 75-octet limit.
            const fold = (s) => { let out = '', line = '', bytes = 0; for (const char of s) {
...
export function resultCSV(r) {
    const cell = (v) => '"' + (/^[\s]*[=+@\-\t\r]/.test(v) ? "'" + v : v).replaceAll('"', '""') + '"';
    if (!r.columns || !r.rows)
        throw new Error('Ehhez az eredményhez nincs táblázat.');
