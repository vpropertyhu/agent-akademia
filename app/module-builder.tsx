...
import {builderCardHelp,builderRepairCards,builderSelection,maxBuilderCards,maxPilotCards} from '../lib/card-guidance';
...
export default function ModuleBuilder(){
 ...
 const cardSelection=useMemo(()=>builderSelection(draft.pieces),[draft.pieces]);
 const trialProblems=[...cardSelection.problems,...(problems.length?['Javítsd a fent jelzett sorrendet is.']:[]),...(!draft.title.trim()?['Adj nevet a segítődnek.']:[]),...((draft.request||'').trim().length<3?['Írd le a mezőbe, mit készítsen el.']:[])];
 ...
 function change(next:Draft,message:string,restoreOlder=false){
  if(!restoreOlder&&next.pieces.length>maxBuilderCards&&next.pieces.length>draft.pieces.length){setNotice(`Legfeljebb ${maxBuilderCards} kártya fér a mezőbe. Az együtt mentett lépések különválasztásához is kell hely.`);return false;}
 ...
 function add(blockId:string,index=draft.pieces.length,module=false){
  if(draft.pieces.length>=maxBuilderCards){setNotice(`Legfeljebb ${maxBuilderCards} kártyát tehetsz a mezőbe. Vegyél ki egyet, vagy ments el több lépést együtt.`);return;}
  ...
  if(draft.pieces.length+additions.length>maxBuilderCards){setNotice('A szükséges kezdőkártyának is kell hely. Előbb vegyél ki egy kártyát.');return;}
 ...
 function repair(index:number){
  const ids=builderRepairCards(draft.pieces,index);if(!ids.length)return;
  if(draft.pieces.length+ids.length>maxBuilderCards){setNotice('A javításhoz több hely kell. Előbb vegyél ki kártyát, vagy ments el néhány lépést együtt.');return;}
 ...
 return <div className="module-builder" onDragEnd={()=>{setDragged(null);setDropAt(null);setOverCanvas(false);}} onKeyDown={e=>{if(e.key==='Escape'){setDragged(null);setDropAt(null);setOverCanvas(false);setGrouping(false);setRecipesOpen(false);}}}>
  <header className="mb-header">
   ...
   <nav aria-label="Főmenü"><a className="mb-nav-active" href="/">Segítő összeállítása</a><a href="/proba">Próbálj ki egy segítőt <ArrowUpRight size={14}/></a><a href="/alkotas">Szövegalkotó <ArrowUpRight size={14}/></a><button onClick={()=>setHelp(!help)} aria-expanded={help}><BookOpen size={16}/> Segítség</button></nav>
  ...
  <main className="mb-workbench mb-single-field">
   <aside ref={catalogRef} className="mb-palette" aria-label="Feladat választása">
    ...
    <div className="mb-palette-list">
     {palette.map(b=>{const help=builderCardHelp(b);return <button key={b.id} className={`mb-palette-block mb-family-${b.family}`} disabled={draft.pieces.length>=maxBuilderCards} draggable={draft.pieces.length<maxBuilderCards} onDragStart={e=>{e.dataTransfer.setData('text/plain',b.id);setDragged({block:b.id});}} onClick={()=>add(b.id,targetIndex)}><span className="mb-mini-icon"><BlockIcon block={b}/></span><span><strong>{b.name}</strong><small>{b.verb}</small><span className={'mb-card-availability '+(help.supported?'':'mb-card-later')}>{help.supported?'A próbához támogatott':'Itt még nem futtatható'}</span><small className="mb-card-requirement">{help.requirement}</small>{b.inputs.length>0&&<small>{help.supported?'A webes próbához például:':'Például:'} {help.minimum} külön kártya.</small>}{b.needs&&<em>{b.needs}</em>}</span><Plus size={18}/></button>;})}
     {customPalette.map(m=>{return <button key={m.id} className="mb-palette-block mb-family-sajat" disabled={draft.pieces.length>=maxBuilderCards} draggable={draft.pieces.length<maxBuilderCards} onDragStart={e=>{e.dataTransfer.setData('text/plain',m.id);setDragged({module:m.id});}} onClick={()=>add(m.id,targetIndex,true)}><span className="mb-mini-icon"><Layers3 size={23}/></span><span><strong>{m.name}</strong><small>{`${flatten(m.children).length} lépés együtt`}</small></span><Plus size={18}/></button>;})}
   ...
   <section className="mb-stage" aria-label="A kiválasztott feladatok">
    ...
    <section className="mb-card-rules" aria-label="A kártyák feltételei"><div><strong>Minimum: kezdés + feladat</strong><p>Például Írd le, mit szeretnél + Szövegírás. Ez 2 kártya, de csak 1 AI-feladat.</p></div><div><strong>Maximum: {maxBuilderCards} kártya a tervben</strong><p>A webes próbában 1–{maxPilotCards} különböző AI-feladat használható, mindegyik egyszer. Az együtt mentett lépések is beleszámítanak.</p></div></section>
    <div className="mb-field-heading"><span><span className="mb-field-number">01</span> Ide gyűjtöd a feladatokat</span><span>{draft.pieces.length} / {maxBuilderCards} kártya · {cardSelection.steps.length} / {maxPilotCards} AI-feladat a próbához</span></div>
    {draft.pieces.length>maxBuilderCards&&<p className="mb-old-limit">Ez egy korábban mentett, nagyobb terv. Módosíthatod és csökkentheted; új kártyát a 40-es kereten belül adhatsz hozzá.</p>}
    {!draft.pieces.length&&<div className="mb-starting-cards"><span>Előkészítjük a két szükséges kártyát:</span><button onClick={()=>add('write')}><Plus size={16}/> Szövegíró segítő</button><button onClick={()=>add('tasks')}><Plus size={16}/> Teendőlistás segítő</button></div>}
    ...
    <div ref={canvasRef} className={`mb-canvas mb-drop-field ${overCanvas?'mb-field-hover':''} ${grouping?'mb-grouping':''} ${!draft.pieces.length?'mb-field-empty':''}`} aria-label="Ide tedd a kiválasztott feladatokat" onDragOver={e=>{if(dragged){e.preventDefault();e.dataTransfer.dropEffect=dragged.piece?'move':'copy';setOverCanvas(true);}}} onDragLeave={e=>{if(!(e.relatedTarget instanceof Node)||!e.currentTarget.contains(e.relatedTarget))setOverCanvas(false);}} onDrop={e=>drop(e,draft.pieces.length)}>
     ...
     {!draft.pieces.length?<div className="mb-empty-field-content"><span className="mb-empty-target"><Plus size={35}/></span><h2>Ide kerülnek a választott feladatok</h2><p>Kattints például a <strong>Szövegírás</strong> melletti + jelre.<br/>A kártyát ide is húzhatod.</p><span className="mb-empty-caption">Először kiválasztod, miben segítsen. Utána végigvezetünk a kipróbáláson.</span></div>:<>
      ...
      <section className="mb-assembly-summary" aria-label="A kiválasztott feladatok összefoglalója"><div className="mb-summary-heading"><div><span className="mb-eyebrow">EZEKET VÁLASZTOTTAD</span><h2>A segítőd feladatai</h2></div><span>{draft.pieces.length} elem<br/><strong>{leaves.length} lépés</strong></span></div>
       ...
       {problems.length>0?<div className="mb-summary-warning"><strong>A folytatáshoz ezen még igazítani kell:</strong>{draft.pieces.every((p,i)=>connection(draft.pieces[i-1],p).ok)&&<p>Egy együtt mentett kártyán belül nem jó a sorrend. Kattints rá, majd válaszd a „Lépések különválasztása” gombot.</p>}{draft.pieces.map((p,i)=>{const link=connection(draft.pieces[i-1],p);if(link.ok)return null;const repairIds=builderRepairCards(draft.pieces,i);return <div key={p.uid}><p>{definition(p).name}: {link.message}</p>{repairIds.length>0&&<button onClick={()=>repair(i)}><Plus size={15}/>{repairIds.map(id=>blocks.find(b=>b.id===id)!.name).join(' + ')} hozzáadása</button>}</div>;})}</div>:cardSelection.hasTask?<p className="mb-summary-ok"><Link2 size={17}/>A terv kártyái egymáshoz kapcsolódnak.</p>:<p className="mb-summary-warning">A kezdés már megvan. Válassz mellé egy feladatot is.</p>}
       <section className={'mb-trial-readiness '+(trialProblems.length?'mb-trial-incomplete':'')} aria-live="polite" aria-label="Mi kell még a webes kipróbáláshoz?"><strong>{trialProblems.length?'A webes kipróbáláshoz még ez kell:':'Az összeállítás átvihető a webes próbába.'}</strong>{trialProblems.length?<ul>{trialProblems.map(p=><li key={p}>{p}</li>)}</ul>:<p>Az AI-kapcsolat állapotát a következő oldalon látod. A leírás és a dokumentumkártya nem számít külön AI-feladatnak.</p>}{cardSelection.unsupported.length===0&&!cardSelection.steps.some(id=>id==='write'||id==='tasks')&&<div><button onClick={()=>add('write')}><Plus size={16}/> Szövegírás hozzáadása</button><button onClick={()=>add('tasks')}><Plus size={16}/> Teendőlista hozzáadása</button></div>}</section>
       <div className="mb-use-next"><div><strong>A saját összeállításoddal folytatod.</strong><p>{trialProblems.length?'A kártyákat továbbra is szabadon módosíthatod. A webes próbához pótold a fent jelzett feltételeket.':'A következő oldalon még módosíthatsz a kártyákon. Az AI csak akkor indul, amikor ott a Kipróbálom gombra kattintasz.'}</p></div><button disabled={trialProblems.length>0} onClick={runHere}>Ezt az összeállítást próbálom ki <ArrowUpRight size={19}/></button><button disabled={problems.length>0||!cardSelection.hasTask} onClick={openHandoff}>Kézi átadás másik AI-nak</button></div>
      </section>
     </>}
    </div>
  ...
  <p className="mb-export-explanation">A letöltött összeállítást később itt nyithatod meg. A használathoz válaszd a webes kipróbálást vagy a „Kézi átadás másik AI-nak” gombot.</p>
 </div>;
}
